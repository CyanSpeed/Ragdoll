...
export async function generateSearchItems(
  ...
  try {
    const apiGroups = await context.globalState.get("apiGroups");
    ...
    const selectApi = await vscode.window.showQuickPick(apiSelectList, {
...
const getFolderSelectList = (apiGroups: any[]) => {
...
const getApiSelectList = (folder: any[]) => {
  const apiList = folder.list.map((item) => {
    return {
      label: item.title,
